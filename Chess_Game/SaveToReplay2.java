...
/**
 * The class SaveToReplay2 will check if the SaveToReplay2 button has been pressed, then call on methods that will save the game to a file.
 * 
 * @author Michael Tuccillo
 * @version 1.0
 * @since 2022-1-14
 */
public class SaveToReplay2 extends Buttons
{
    /**
     * This Act method is called constantly and will check to see whether or not the SaveToReplay2 button was pressed.
    ...
    public void act() {
        ...
        if (Greenfoot.mousePressed(this) && (getWorld().getObjects(CheckMark.class).size() == 0)) {
            ...
            ((ChessBoard)getWorld()).copyList(2);
            // Call on method to create the check mark, pass in y value for location
            ((ChessBoard)getWorld()).createCheckMark(645);
        }
    }
}
